...
def merge(eval_path, num_tasks, method="prob"):
    ...
    input_lst = []
    results = {"video_object": [], "predict": []}
    for i, item in enumerate(dict_feats):
        input_lst.append([i, item, dict_feats[item], dict_label[item]])
        results["video_object"].append(item)
        feat = [x for x in dict_feats[item]]
        feat = np.mean(feat, axis=0)
        results["predict"].append(np.argmax(feat))
    p = Pool(64)
    ...
    results_df.to_csv("./tracking_dataset/dance.csv", index=False)
